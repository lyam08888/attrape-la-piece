...
let images = {}, imagesToLoad = 4, imagesLoaded = 0;
["player","coin","enemy","flag"].forEach(name => {
  images[name] = new Image();
  images[name].src = `assets/${name}.png`;
  images[name].onload = () => { imagesLoaded++; if(imagesLoaded === imagesToLoad) startGame(); }
});

let PARALLAX_COUNT = 5;
let game, keys = {};
...
function draw() {
  ...
  ctx.drawImage(images.flag, game.flag.x, game.flag.y, 24, 24);
  let p = game.player;
  ctx.save();
  ctx.translate(p.x + p.w/2, p.y + p.h/2);
  ctx.scale(p.dir,1);
  ctx.drawImage(images.player, -12, -12, 24, 24);
  ctx.restore();
  ctx.restore();
}
