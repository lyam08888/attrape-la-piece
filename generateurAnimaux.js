    ...
    update(game) {
        ...
        if (this.x < player.x + player.w && this.x + this.w > player.x && this.y < player.y + player.h && this.y + this.h > player.y) {
            if (player.vy > 0 && (player.y + player.h) < (this.y + this.h / 2)) {
                this.isDead = true;
                player.vy = -player.config.jumpForce * 0.6;
...
// --- NOUVEAU: CLASSE ANIMAL ---
class Animal {
    constructor(x, y, config, animalData) {
        this.x = x;
        this.y = y;
        this.w = config.tileSize * 0.8; // Un peu plus petits
        this.h = config.tileSize * 0.8;
        this.properties = animalData;
        this.isDead = false; // Pour une future extension (chasse, etc.)

        this.vx = 0;
        this.vy = 0;
        this.speed = config.tileSize * (0.005 + Math.random() * 0.015);
        this.direction = Math.random() < 0.5 ? 1 : -1;
        this.actionTimer = 120 + Math.random() * 240; // Moins actifs que les monstres
    ...
    update(game) {
        ...
        // Comportement basé sur le type de mouvement
        if (this.properties.movement === 'fly') {
            this.vy = (Math.random() - 0.5) * 0.5; // Vole de manière erratique
        } else if (this.properties.movement === 'swim') {
            this.vy = (Math.random() - 0.5) * 0.3; // Nage doucement
            this.vx *= 1.5; // Les poissons sont plus rapides
        } else { // 'walk'
            this.vy += physics.gravity; // Les animaux terrestres sont soumis à la gravité
            if (this.vy > physics.maxFallSpeed) this.vy = physics.maxFallSpeed;
        }
    ...
    handleCollisions(game, tileSize) {
        // Logique de collision simplifiée pour les animaux
        this.x += this.vx;
        this.y += this.vy;

        // Empêche de sortir du monde (simple)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        game = {
            ...
            tileMap: [],
            enemies: [],
            animals: [], // NOUVEAU: Tableau pour les animaux
            particles: [],
            fallingBlocks: [],
            collectibles: [],
            decorations: [],
    ...
    // NOUVEAU: Logique d'apparition des animaux
    function spawnAnimals() {
        ...
        if (Math.random() < 0.015) { // Un peu plus fréquents que les monstres
            ...
            if (animalData.movement === 'fly') {
                 const newAnimal = new Animal(spawnX, spawnY - 100, config, animalData); // Apparaît dans le ciel
    ...
    function update(keys, mouse) {
        ...
        try {
            ...
            game.animals.forEach(a => a.update(game)); // NOUVEAU: Mise à jour des animaux
            updateParticles();
            updateFallingBlocks();
            updateCollectibles();
            ...
            spawnMonsters();
            spawnAnimals(); // NOUVEAU: Appel à la logique d'apparition des animaux
